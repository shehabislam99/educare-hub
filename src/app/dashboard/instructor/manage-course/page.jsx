        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 font-medium">
              Manage and optimize your active course deployments.
            </p>
          </div>
          <Link
            href="/dashboard/instructor/create-course"
            className="bg-indigo-600 text-white font-bold py-4 px-8 rounded-2xl hover:bg-indigo-700 transition-all shadow-md shadow-indigo-50 flex items-center gap-2 group"
          >
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden min-h-[500px]">
          <div className="p-8 border-b border-slate-50 flex flex-col sm:flex-row gap-6 justify-between items-center text-slate-900">
            <div className="relative w-full sm:w-96">
              ...
              <input
                type="text"
                placeholder="Filter your Course in database..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full bg-slate-50 border border-slate-300 rounded-2xl pl-12 py-3.5 text-sm font-bold focus:ring-2 focus:ring-indigo-500 transition-all"
              />
            </div>
            <div className="flex gap-4 text-lg font-black uppercase  text-slate-400">
              Total Course:{" "}
                ...
                {isLoading ?
                  [...Array(3)].map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      <td
                        colSpan={5}
                        className="px-8 py-10 h-16 bg-slate-50/20"
                      />
                    </tr>
                  ))
