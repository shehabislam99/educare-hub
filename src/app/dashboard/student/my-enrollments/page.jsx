...
export default function MyEnrollments() {
    ...
    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-3xl font-bold text-slate-900 mb-2">My Enrolled Courses</h1>
                <p className="text-slate-500">Continue where you left off and master new skills.</p>
            </div>
                ...
                {enrolledCourses.length === 0 ? (
                    <div className="bg-white p-20 rounded-3xl border border-slate-100 text-center space-y-6 shadow-sm">
                        ...
                        <div className="max-w-sm mx-auto">
                            <p className="text-xl font-bold text-slate-900">No active enrollments</p>
                            <p className="text-slate-500 mt-2">You haven't enrolled in any courses yet. Start exploring our catalog today!</p>
                        </div>
                        <Link href="/course" className="inline-block bg-indigo-600 text-white px-8 py-4 rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">
                            Explore All Courses
                        </Link>
                    </div>
                ) : (
                    enrolledCourses.map((course) => (
                        <div key={course.id || course._id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all group overflow-hidden relative">
                                    ...
                                    <div>
                                        <div className="flex justify-between items-start mb-4">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">Completed</p>
                                            </div>
                                        </div>

                                        <p className="text-sm text-slate-500 mb-6 italic line-clamp-1">
                                            Next up: <span className="font-semibold text-slate-700">{course.nextLesson || "Getting Started"}</span>
                                        </p>
                                    </div>
